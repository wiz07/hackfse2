...
import ListAllCompany from './ListAllCompany';
...
const UserHome = () => {
    ...
    const [showdeletCompany, setshowdeletCompany] = useState(false);
    const [addNewCompany, setaddNewCompany] = useState([])
    const [deletCompany, setdeletCompany] = useState([])
    ...
    // })
    // }, [])
    const listCompanyHandler = () => {
        setshowListCompany(true);
        setshowAddCompany(false)
        setshowCompanyInfo(false)
        setshowdeletCompany(false)
    }
    ...
    const deleteCompanyHandler = () => {
        ...
        setshowAddCompany(false)
        setshowCompanyInfo(false)
        setshowdeletCompany(true)

    }
    ///IMP!!!
    stocks.push(addNewCompany)
    // const indexOfStock = stocks.findIndex(obj => {
    //     return obj.symbol == deletCompany
    // })
    // console.log(indexOfStock)
    // stocks.splice(indexOfStock, 1)
    return (
        <>
            {/* <div>normal user</div> */}
            <div
                ...
                <button
                    ...
                    }}>Delete Company Data</button>
                <br />
            </div>

            {showListCompany ? <ListAllCompany
                stocks={stocks} /> :
                showAddCompany ? <AddCompany addNewCompany={addNewCompany} setaddNewCompany={setaddNewCompany} /> :
                    showCompanyInfo ? <SearchCompanyInfo
                        stocks={stocks}
                    /> : showdeletCompany ? <DeleteCompany
                        deletCompany={deletCompany} setdeletCompany={setdeletCompany} stocks={stocks}
                    /> : <div
                        style={{
